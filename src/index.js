...
const updateSpaces = require('./spaces')
...
program.command('deploy')
  ...
  .option('--space <space>', 'Apigee Space to assign this proxy to (Apigee X/Hybrid only)')
  .description('Deploy a proxy based on a folder')
  .action((options) => deployProxy(build(), options.api, options.directory, options.serviceaccount, options.space).catch(handleError))
...
program.command('deploySharedFlow')
  ...
  .option('-sa, --serviceaccount <serviceaccount>', 'The serviceAccount attached to this proxy deployment, can be used to retrieve google Access tokens with the serviceCallout policy')
  .option('--space <space>', 'Apigee Space to assign this shared flow to (Apigee X/Hybrid only)')
  ...
  .action((options) => deploySharedFlow(build(), options.sf, options.directory, options.serviceaccount, options.space).catch(handleError))
...
program.command('spaces <manifest>')
  .description('Creates or updates Apigee Spaces and their IAM member bindings based on the given manifest')
  .action((manifest) => updateSpaces(build(), manifest).catch(handleError))
